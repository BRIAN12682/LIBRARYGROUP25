...
class SaveCategory(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        try:
            if id > 0:
                category = models.Area.objects.exclude(id = id).get(name = name, delete_flag = 0)
            else:
                category = models.Area.objects.get(name = name, delete_flag = 0)
        except:
            return name
        raise forms.ValidationError("Category Name already exists.")


class SaveSubArea(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        try:
            category = models.Area.objects.get(id = cid)
            if id > 0:
                sub_category = models.SubArea.objects.exclude(id = id).get(name = name, delete_flag = 0, category = category)
            else:
                sub_category = models.SubArea.objects.get(name = name, delete_flag = 0, category = category)
        except:
            return name
...
class SaveBook(forms.ModelForm):
    ...
    def clean_sub_category(self):
        ...
        try:
            sub_category = models.SubArea.objects.get(id = scid)
            return sub_category
        except:
...
class SaveIssueBook(forms.ModelForm):
...
class RegistrationForm(UserCreationForm):
    ...
    Class = forms.CharField(max_length=15)

    class Meta:
        model = User
        fields = ['first_name', 'last_name', 'username', 'password1', 'password2', 'Email', 'Gender', 'College', 'Class']
